...
const SeatChart = ({ occasion, tiX, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await tiX.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await tiX.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
        ...
        {!seatsTaken && 
          ...
          {Array(25).fill(1).map((_, i) => (
              <div
                  ...
                  style={{
                      gridColumn: `${(i % 5) + 1}`,  // 5 columns
                      gridRow: `${Math.floor(i / 5) + 1}` // Rows increment every 5 seats
                  }}
              >
                  {i + 1} {/* Seat number */}
              </div>
